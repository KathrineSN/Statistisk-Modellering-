...
plot(sigma1,exp(-(pnll-min(pnll))),type="l", ylim=c(0,1), xlim = c(0,0.2), xlab = 'Standard deviation', ylab = 'Profile likelihood')
lines(range(sigma1),
...
plot(sigma1,exp(-(pnll-min(pnll))),type="l", ylim=c(0,1), xlim = c(0,0.2), xlab = 'Standard deviation', ylab = 'Profile likelihood')
lines(range(sigma1),
      c(1,1)*exp(-qchisq(0.95,df=1)/2), ylim=c(0,1), xlim = c(0,0.2), col=2,lty=2,lwd=2)
...
plot(seq(min(data$SLV), max(data$SLV), 0.001), sapply(seq(min(data$SLV), max(data$SLV), 0.001), mix.dist.new, delta=delta, npars=natural_params),ylab = "Likelihood",xlab="Weekly return", 
     type="l", lwd = 4, main=paste(c("One Step Distribution - State",j), collapse = " "))
   ## Samlede distribution
  
  for (i in 1:3){
   npars <- list("mu"=natural_params$mu[i],"sigma"=natural_params$sigma[i])
    lines(seq(min(data$SLV), max(data$SLV), 0.001), sapply(seq(min(data$SLV), max(data$SLV), 0.001), mix.dist.new, delta=delta[i], npars=npars),col=1+i, lwd = 4)}
