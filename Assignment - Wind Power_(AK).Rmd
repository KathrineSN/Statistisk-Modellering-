...
lp.lambda1 <- function(lambda,y){
    ...
    y.l <- trans1(lambda ,y)
    sigmasq <- 1/n * sum((y.l-mean(y.l))^2)
    -n/2 * log(sigmasq) + (lambda-1)*sum(log(y))}  #Sidste del er profile likelihood for coxbox, skal findes for den selv.

lambda <- seq(0,2,by=0.01)
    
(opt.lambda1=optimize(lp.lambda1,c(0,5),y=data$pow.obs.norm,maximum=TRUE))
```
