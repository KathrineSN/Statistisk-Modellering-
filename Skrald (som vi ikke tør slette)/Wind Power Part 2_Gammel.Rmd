...
lp.lambda1 <- function(lambda,y){
    ...
    #Første led kommer fra normalfordeling og andet 
    #Change of variable bruges til at finde den optimale lambda
    }
...
## profile likelihood for lambda
lp.lambda1 <- function(lambda,y){
    mod <- lm(trans1(lambda,y)~data$ws30+I(data$ws30^2)+I(data$ws30^3))
    length(y)/2*log(summary(mod)$sigma^2) - sum(log(abs(1/(y*(1 - y^lambda)))))
    }
...
#HVORFOR ER 2.GRADSLEDDET IKKE SIGNIFIKANT HER?
