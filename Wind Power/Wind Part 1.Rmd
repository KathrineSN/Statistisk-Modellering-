...
$$lp(\lambda)=\frac{-n}{2} \cdot log(\sigma^{2}) + \sum\frac{-1}{(y\cdot(-1 + y^{\lambda}))}$$
...
xfit<-seq(min(x),max(x),length=40)
yfit <- dvonmises(xfit,Vonmieses.function(data$wd30)$opt$par[1],Vonmieses.function(data$wd30)$opt$par[2])
yfit <- yfit*diff(h$mids[1:2])*length(x)
lines(xfit, yfit, col="blue", lwd=2)
```
