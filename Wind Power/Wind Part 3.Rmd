...
(opt.ar1 <- nlminb(c(pars[1],pars[2]), nll.ar1, e=e,lower = c(-Inf,-Inf), upper=c(Inf,Inf)))

phi1 <- (exp(2*opt.ar1$par[2])-1)/(1+exp(2*opt.ar1$par[2]))
(phi1)
# ca. 0.3
...
(opt.ar1.full <- nlminb(c(log(pars[1]),pars[2]), nll.ar1.full, e=e,lower = c(-Inf,-Inf), upper=c(Inf,Inf)))
...
(AIC = 2*2-2* log(opt.ar1$objective))
